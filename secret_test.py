...
def validate_secret(secret, n):
    """Validate secret(x) + secret(y) == secret(x + y) for all primes < n
    ...
    Returns:
      Bool: False if secret(x) + secret(y) != secret(x+y) for some primes \
x and y, otherwise True
    ...
    for prime in _get_primes(n):
        try:
            if not all(secret(p + prime) ==
                       secret(p) + secret(prime) for p in primes):
